...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    std::cout << std::endl; // Formating new line between each detection. - JS
  // clear any prior results
  records.clear();

  /***  Stop SQL Injection of type Value = Value; <-Semicolon being the main check.  ***/

  std::string lowercaseSql(sql); // Creating lowercase copy of input query. - JS

  std::transform(lowercaseSql.begin(), lowercaseSql.end(), lowercaseSql.begin(), ::tolower); // Utilizing C++ transfom to transform entire SQL string to lowercase. - JS

  size_t whereClausePosition = lowercaseSql.find(str_where); // Finding position of where clause in the lowercaseSql string. - JS

  if (whereClausePosition != std::string::npos) { // Conditional statement to check if the where clause is found. npos used to find maximun possible value of size_t whereClausePosition. - JS
      size_t semiColonPosition = lowercaseSql.find(';', whereClausePosition); // Checking if semicolon is found at the end of where clause. - JS
    
      if (semiColonPosition != std::string::npos && semiColonPosition > whereClausePosition) { // Condition statement to check if semicolon is found. - JS
          std::cout << "System Detected A Potential SQL Injection Attack: " << sql << std::endl; // Output message to terminal with SQL indicating a semicolon is found. - JS
          return false; // Return false to end Sql query. - JS
      }
  }
