...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // Overflow check - JS
        if (numeric_limits<T>::max() - result < increment) { // Determining if the addition of the increment to the current value will cause Overflow - JS
            return numeric_limits<T>::max(); // Returning Max Value of type T - JS
        }
        result += increment;
    }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result < decrement) { // is result is less than decrement an underflow is occuring. - JS
            return numeric_limits<T>::min(); // return min value of type T - JS
        }
        result -= decrement;
    }
...
void test_overflow()
{
    ...
    //REMOVED OUTPUT STATEMENT - JS

    // ADDED CODE - JS

    if (result == numeric_limits<T>::max()) { // If the result is equal to the max value of type T an overflow occured. - JS
        cout << "Overflow Found @ result = " << +result << endl; // Output message to user that an overflow occured & display results. - JS
    }
    else {
        cout << "+result" << endl; // if no overflow display results - JS
    }
}
...
void test_underflow()
{
    ...
    // REMOVED OUTPUT OF VALUE - JS

    if (result == numeric_limits<T>::min()) { // If the result is equal to the min value of type T and underflow occured - JS
        cout << "Underflow Found @ result = " << +result << endl; // Output results if underflow is found to user with results. 
    }
    else {
        cout << +result << endl; // if not underflow is found display results. - JS
    }
}
